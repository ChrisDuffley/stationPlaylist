...
class breakNoteDialogOverlay(NVDAObjects.NVDAObject):
	...
	def editElementFavourites(self, parent, elements):
		...
		favouriteListLabel = wx.StaticText(
			dialog,
			label="&Select favourite break notes:",
		)
		dialogSizer.Add(favouriteListLabel, 0, wx.LEFT | wx.RIGHT | wx.TOP, 10)
		...
		def toggleFavourite(event):
			# wx.ListBox does not provide a built-in checkable mode, so the
			# checked state is kept separately and represented in each label.
