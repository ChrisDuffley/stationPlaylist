...
def loadHelpTexts(path=HELP_TEXTS_FILE):
  helpTexts = {}
  currentKey = None
  currentLines = []
...
def loadElements(path=ELEMENTS_FILE):
  ...
  filterSelection = elementValues.get(FILTER_SELECTION_KEY, 0)
  if (
    not isinstance(filterSelection, int)
    or isinstance(filterSelection, bool)
    or filterSelection not in (0, 1)
  ):
...
def saveElementValues(elements, path=ELEMENT_VALUES_FILE):
  values = {}
  ...
  values[FILTER_SELECTION_KEY] = filterSelection
...
def saveElementFavourites(elements, path=ELEMENTS_FILE):
  with path.open(encoding="utf-8") as elementsFile:
    values = json.load(elementsFile)
...
def editElementFavourites(parent, elements):
  ...
  try:
    ...
    saveElementFavourites(elements)
    return True
  finally:
    dialog.Destroy()


def getNumberValue(parent, element):
  kind = element.kind
  ...
  while True:
    ...
    isWholeNumber = (
      bool(enteredValue)
      and enteredValue.lstrip("+-").isdigit()
      and enteredValue not in ("+", "-")
    )
    if allowEmpty and not enteredValue:
      return ""
    numberValue = int(enteredValue) if isWholeNumber else None
...
def getTextValue(parent, element):
...
def getTypeAndPathValue(parent, element):
...
def getPlayerVolumeValue(parent):
...
def getCartValue(parent, breakNoteCode, element):
...
def getRecordValue(parent, element):
  duration = getNumberValue(parent, element)
  if duration is None:
    return None
...
def getHookValue(parent, element):
  ...
  trackNumber = getNumberValue(parent, element)
  if trackNumber is None:
    return None
  return f"={hookPrefix}{trackNumber}"


def getDSPValue(parent):
  while True:
...
def showBreakNoteDialog(elements):
  dialog = wx.Dialog(
    gui.mainFrame,
  ...
  elementFilter.SetSelection(filterSelection)
  ...
  def updateElementList(event=None):
    ...
    filterSelection = elementFilter.GetSelection()
    saveElementValues(elements)
    selectedElementID = (
    ...
    visibleElements = [
      ...
      if filterSelection == 0 or element.isFavourite
    ]
    elementList.SetItems(
      [getElementLabel(element) for element in visibleElements]
    )
    selectedIndex = next(
      (
        elementIndex
  ...
  def editFavourites(event):
    if editElementFavourites(dialog, elements):
      updateElementList()
    event.Skip()
  ...
  def showElementMenu(event):
    ...
    if selectedElement.code == "PlayerVol":
      value = getPlayerVolumeValue(dialog)
      if value is not None:
        selectedElement.value = value
      return
    if selectedElement.code in ("C", "O"):
      value = getCartValue(dialog, selectedElement.code, selectedElement)
      if value is not None:
        selectedElement.value = value
      return
    if selectedElement.ID == 46:
      value = getRecordValue(dialog, selectedElement)
      if value is not None:
        selectedElement.value = value
      return
    if selectedElement.ID == 27:
      value = getHookValue(dialog, selectedElement)
      if value is not None:
        selectedElement.value = value
      return
    if selectedElement.code == "Dsp":
      value = getDSPValue(dialog)
      if value is not None:
        selectedElement.value = value
      return
    if selectedElement.type == bnType.onOff:
      ...
      elementList.SetString(
        elementList.GetSelection(), getElementLabel(selectedElement)
      )
      return
    ...
    if selectedElement.type in (bnType.typeAndDir, bnType.typeAndFile):
      value = getTypeAndPathValue(dialog, selectedElement)
      if value is None:
        return
    ...
    if selectedElement.type == bnType.number:
      numberValue = getNumberValue(dialog, selectedElement)
      if numberValue is not None:
        selectedElement.value = numberValue
      return
    if selectedElement.type == bnType.text:
      enteredText = getTextValue(dialog, selectedElement)
...
def createTextFromBreakNote(element):
  if element.code == "":
...
def createBreakNote(callback, elements=None):
  if elements is None:
    elements = loadElements()

  selectedElement = showBreakNoteDialog(elements)
  result = (
    createTextFromBreakNote(selectedElement)
