...
# Copyright 2011, 2013-2018, Geoff Shang, Joseph Lee and others, released under GPL.
...
class SPLTrackItem(IAccessible):
	...
	def _get_locationText(self):
		...
		return _("Item {current} of {total}").format(current = self.IAccessibleChildID, total = studioAPI(0, 124))
...
class AppModule(appModuleHandler.AppModule):
	...
	def script_sayRemainingTime(self, gesture):
		if studioIsRunning(): self.announceTime(studioAPI(3, 105), offset=1)
	...
	def _trackFinderCheck(self, attemptLevel):
		...
		elif api.getForegroundObject().windowClassName == "TStudioForm" and not studioAPI(0, 124):
	...
	def monitorLibraryScan(self):
		...
		if studioAPI(1, 32) < 0:
			...
			ui.message(_("{itemCount} items in the library").format(itemCount = studioAPI(0, 32)))
		else:
	...
	def libraryScanReporter(self):
		...
		if splconfig.SPLConfig["General"]["LibraryScanAnnounce"] != "off":
			if splconfig.SPLConfig["General"]["BeepAnnounce"]:
				...
				ui.message(_("Scan complete with {itemCount} items").format(itemCount = studioAPI(0, 32)))
	...
	def playlistSnapshots(self, obj, end, snapshotFlags=None):
		...
		while obj not in (None, end):
			...
			if segue not in (None, "00:00"):
				...
				if len(hms) == 3: totalDuration += int(hms[0])*3600
			obj = obj.next
		if end is None: snapshot["PlaylistItemCount"] = studioAPI(0, 124)
...
# Pressed once will speak and/or braille it, pressing twice or more will output this info to an HTML file.
	...
	def sayStatus(self, index, statusText=False):
		if self.SPLCurVersion < "5.20":
			...
			status = self._statusBarMessages[index][studioAPI(index, 39)]
