...
# Copyright 2011, 2013-2026 Geoff Shang, Joseph Lee and others, released under GPL.
...
class AppModule(splappmod.AppModule):
	...
	def chooseNVDAObjectOverlayClasses(self, obj: NVDAObject, clsList: list[NVDAObject]) -> None:
		...
		if breakNoteDialogAllowed(obj, "TMemo") and SPLBreakNoteEntryField not in clsList:
			...
			# Track insert form/library scan announcement.
				...
				if "match" in obj.name:  # Status text 1
					...
					if (
						...
						if splconfig.SPLConfig["General"]["BeepAnnounce"]:
							tones.beep(370, 100)
							ui.message(f"{splbase.studioAPI(0, splbase.SPLLibraryScanCount)}")
						else:
							ui.message(
		...
		match trackTime:
			case "remaining":
				...
				if cartRemainingTime > 0:
					ui.message(f"{self._ms2time(cartRemainingTime, offset=1)} (cart)")
				elif vtRemainingTime > 0:
					ui.message(f"{self._ms2time(vtRemainingTime, offset=1)} (voice track)")
				else:
	...
	# Track finder
	# Internals defined in splfind module.
	...
	def libraryScanReporter(self) -> None:
		...
		if splconfig.SPLConfig and splconfig.SPLConfig["General"]["LibraryScanAnnounce"] != "off":
			if splconfig.SPLConfig["General"]["BeepAnnounce"]:
				tones.beep(370, 100)
				wx.CallAfter(ui.message, f"{splbase.studioAPI(0, splbase.SPLLibraryScanCount)}")
			else:
	...
	# SPL Assistant commands (script, NVDA layout, JAWS layout)
	__SPLAssistantGestures = (
		("sayPlayStatus", "kb:p", "kb:p"),
		("sayAutomationStatus", "kb:a", "kb:a"),
		("sayMicStatus", "kb:m", "kb:m"),
		("sayLineInStatus", "kb:l", "kb:shift+l"),
		("sayRecToFileStatus", "kb:r", "kb:shift+e"),
		("sayCartEditStatus", "kb:t", "kb:0"),
		("sayControlKeysStatus", "kb:control+d", "kb:control+d"),
		("sayUpTime", "kb:u", "kb:u"),
		("sayListenerCount", "kb:i", "kb:l"),
		("sayTrackPitch", "kb:shift+p", "kb:shift+p"),
		("sayPlaylistModified", "kb:y", "kb:y"),
		("sayHourTrackDuration", "kb:h", "kb:t"),
		("sayHourRemaining", "kb:shift+h", "kb:h"),
		("sayPlaylistRemainingDuration", "kb:d", "kb:r"),
		("sayHourOvertime", "kb:o", "kb:o"),
		("sayScheduledTime", "kb:s", "kb:s"),
		("sayScheduledToPlay", "kb:shift+s", "kb:shift+s"),
		("sayNextTrackTitle", "kb:n", "kb:n"),
		("sayCurrentTrackTitle", "kb:c", "kb:shift+c"),
		("toggleCartExplorer", None, "kb:c"),
		("sayTemperature", "kb:w", "kb:w"),
		("takePlaylistSnapshots", "kb:f8", "kb:f8"),
		("playlistTranscripts", "kb:shift+f8", "kb:shift+f8"),
		("markTrackForAnalysis", "kb:f9", "kb:f9"),
		("trackTimeAnalysis", "kb:f10", "kb:f10"),
		("findTrack", "kb:f", "kb:f"),
		("libraryScanMonitor", "kb:shift+r", "kb:alt+t"),
		("setPlaceMarker", "kb:Control+k", "kb:Control+k"),
		("findPlaceMarker", "kb:k", "kb:k"),
		("metadataStreamingAnnouncer", "kb:e", "kb:e"),
		("switchProfiles", "kb:f12", "kb:f12"),
		("layerHelp", "kb:f1", "kb:f1"),
	)
