...
SPLUpdateURL = "http://www.josephsl.net/files/nvdaaddons/getupdate.php?file=spl-lts16"
...
def initialize():
	...
	try:
		...
		if "UpdateChannel" in SPLAddonState:
			...
			if SPLUpdateChannel not in  ("stable", "lts"):
				SPLUpdateChannel = "lts"
	except IOError:
		SPLAddonState["PDT"] = 0
		_updateNow = False
...
def updateCheck(auto=False, continuous=False, confUpdateInterval=1):
	...
	try:
		...
		# 15.13-LTS: catch ALL exceptions for logging purposes.
