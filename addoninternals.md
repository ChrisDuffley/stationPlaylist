...
Is Studio suite accessible? Surprisingly, yes for the most part. It is possible to use app features without using screen reader scripts and add-ons. However, there are times when a broadcaster would use scripts, such as announcing status changes, monitoring track intros and endings, enhanced support for encoders and so on, and NVDA add-on for StationPlaylist (usually refered to as SPL) accomplishes this well.

Even though Studio suite is accessible, the add-on includes workarounds for issues including labeling unlabeled controls. For example, without the NVDA add-on, track properties dialog in Studio would be harder to navigate because most controls do not have labels. The add-on includes a trick to label unlabeled controls which will be discussed below.

#### SPL apps user interface

Most of the SPL suite consists of a list of tracks and surrounding controls as part of the  main window or the playlist viewer. Local and Remote Studio consists of a list of tracks with checkmarks next to them used to toggle track playback. Track Tool and playlist editor also consists of a list of tracks and surrounding controls except there are no chekcmarks next to tracks. Creator consists of not only tracks list, but also rotations and schedules defined at the local station arranged into tabs.

The controls surrounding the tracks list differ based on the app in use. Local and Remote Studio includes titles of previous, current, and next tracks, timers showing elapsed and remaining times while a track, voice track, or a cart is playing, butotns to toggle microphone and automation, and status bars displaying playback stats, automation status, and for local Studoi, uptime, listener status, and other local Studio management status. In Track Tool, next to tracks list is a group of edit fields used to define intro, outro, hook start and length, and other track playback and property contorls. Creator shows rotations and schedlues.

Some SPL apps include additional screens. For example, local and Remote Studio includes insert tracks dialog to search for and add tracks from the station's library. Local Studoi and Track Tool also includes track properties window to configure properties such as song artist and composer.

Some of the screen controls are labeled via Microsoft Active Accessibility (MSAA), also known as IAccessible while others are not. The NVDA add-on can "label" unlabeled controls by calculating their screen positions and retrieving the label next to the unlabeled control or parsing the actual screen content. This is just one of the workarounds dealing with SPL suite user interface, a result of extensive collaboration between developers and users.
...
More work happened in 2025, and along with it, became the final year of me (Joseph) leading the add-on maintenance. The restructuring work continued beyond 25.06.x releases, resulting in the creation of SPL common services module (documented below). Later, I worked on getting 64-bit NVDA to read Studio track content (it was challenging as SPL suite is 32-bit applications). Most significantly, the introduction of Remote Studio in November 2025 led to the redesign of some of the original Studio app module subsystem (this is because original (local) and the remote versions share some of the user interface as Remote Studio is designed to be a studio on the move and communicate with a local Studio version installed somewhere else). The work on Remote Studio was the last project I personally worked on, and with its completion (or at least having basic support in place), I (Joseph) have retired from active maintainer role in late 2025.
...
StationPlaylist add-on for NVDA consists of nine app modules (including two app module packages), a common services package (housed along with app modules and includes a base blueprint app module to house features common to SPL suite), and a global plugin. Because Studio and Creator come with Track Tool for managing tracks, the add-on includes an app module for Track Tool in addition to the main app module package for Studio, as well as an app module for StationPlaylist Creator. A fourth app module for Voice Track Recorder is present which is used for event tracking purposes. Remote VT client is the fifth app module and is mainly used to support remote playlist editor. The other two app modules deal with Streamer and SPL DSP Engine, with SPL Engine being an app module package due to inclusion of encoders support module which is also used by Streamer. The seventh module, Streamer, does not exist - it is an alias of SPL Engine app module. The eighth module for Remote Studio borrows heavily from the original Studio app module as they share common features and commands. Lastly, StationPlaylist Recorder app module adds labels for unlabeled controls.

As part of the app modules package, a common services collection package (splcommon) is included to house functions, classes, and constants common across the ap modules and the global plugin. Contents include Studio API wrapper, cart key definitions, configuration management, a base app module for the SPL suite, and status messages colection. The common services module resembles Studio app module package in prior add-on releases as majority of its contents indeed come from the Studio app module package, thus the common services serves as an extension of the Studio app module.

The SPL suite base app module housed in SPL common services package provides base services for features found across the SPL suite. Deriving from Studio and Track Tool app modules, it includes configuration opening and closing routines, script category definition for the add-on, and a script to open SPL add-on settings dialog. Any SPL suite app module wishing to use these services are expected to inherit from the base app module.

The overall add-on design is that of a partnership between the main (local) Studio app module and the Studio Utilities (SPLUtils) global plugin. Studio app module performs things expected from scripts such as responding to key presses, announcing status information, configuration management and so forth, while the global plugin is responsible for running Studio commands from anywhere, and in older add-on releases, for encoder support (the add-on supports SAM, SPL, and AltaCast encoders). In reality, the global plugin is subordinate to the app module, as the app module controls overall functionality of the add-on and because the global plugin requires Studio to be running to unlock some features (here, unlock means using layer commands and parts of encoder support).
...
The primacy of the original Studio app module is seen throughout the app modules organization. One case is Remote Studio app module as the latter is the child of the former - not only the way the add-on is organized, but also with the relationship between Studio and Remote Studio promoted by the SPL suite developers. Parts of the original Studio app module were used to create SPL common services base app module for use even by the original Studio app module.
...
For Creator, Remote VT, and Track Tool, it will let you review column data, including playlist editor content in Creator and Remote VT. Due to its purpose, Remote Studio borrows some features from original Studio app module and others are unavailable (for example, metadata stream management feature is unavailable in Remote Studio).
...
Since late 2025, NVDA is a 64-bit program; prior to that, it was a 32-bit program. In contrast, Studio is a 32-bit program, so Studio API returns 32-bit values whereas 64-bit NVDA would prefer 64-bit values. This caused problems in late 2025 while working on 64-bit NVDA support (explained below), leading to a slight change to Studio API wrapper. Before discussing changes made in late 2025, a discussion of how computers represent data and why 32-bit versus 64-bit matter is in order.

A digital computer understands only two things: on or off, represented by binary digits of 1 or 0, respectively. A binary digit (bit) can represent two values on its own (1 or 0), and a collection of them can represent numbers, text, and other values. At the hardware level, a bit is a digital circuit or a switch.

An important data represented by bits is an integer or numbers. Whereas a single bit can represent 1 or 0, a collection can represent an integer range. For example, an 8-bit computer can display integers between 0 and 255, or -128 to +127 if the most significant (leftmost) bit is treated as representing positive (0) or negative (1) numbers. In computer programming, the range 0 through 255 is called unsigned 8-bit integer, whereas the range -128 through +127 is signed 8-bit integer.

Returning to the NVDA versus Studio examples, 32-bit programs such as Studio can work with numbers between 0 and about 4.2 billion for unsigned integers, whereas the signed 32-bit integer range is from -2.1 billion to +2.1 billion. Because Studio is a 32-bit program, Studio API would return signed 32-bit integers. In the 64-bit world such as NVDA, unsigned 64-bit integers can represent values between 0 and about 18.4 quintillion for unsigned 64-bit integer or between -9.2 quintillion to +9.2 quintillion for signed 64-bit integer.

But what happens when a 32-bit program returns a value such as 4.2 billion (more specifically, 4294967295, or 0xffffffff (hexadecimal/base 16 for the highest possible 32-bit integer limit) to a 64-bit program? In the 32-bit world, the value 0xffffffff is either 4294967295 for the unsigned integer or -1 for the signed integer (for the signed integer, the lowest value (-2147483648) is 0x80000000 in hexadecimal). A 64-bit program such as NVDA would see 0xffffffff as 4294967295, a value that cannot be represented in signed 32-bit integer format, leading to what is called an overflow (for a real-life example, search for Year 2038 Problem or Unix time).

The signed 32-bit integer overflow described above was the problem that had to be resolved to let 64-bit NVDA announce values from Studio API correctly. Not only Studio API can return positive numbers or 0 (zero), it can also return negative numbers such as -1 (0xffffffff in signed 32-bit integer representation, 0xffffffffffffffff in signed 64-bit integer representation). For example, playlist overtime can return negative numbers if the tracks loaded do not fill the hour. Values above 0x80000000 is seen as negative numbers by Studio (32-bit) whereas NVDA (64-bit) sees them as positive numbers, so Studio API wrapper was modified in 2025 to convert Studio API return value to signed 32-bit integer for use by the rest of the add-on.
...
Some readers might ask this question after reading the above section on overlay classes. I had to introduce overlay classes because they are important in Studio app module: track items in playlist Viewer are overlay classes. In fact, there are at least three of them: an abstract base class representing track items for studio and other apps (appModules.splcommon.splbase.SPLTrackItem), a generic representation of Studio track item (appModules.splstudio.SPLStudioTrackItem), and a playlist viewer (main window) track item (appModules.splstudio.StudioPlaylistViewerItem; in case of playlist viewer item class, it derives its power from track item class for Studio, which in turn is powered by abstract SPL track item powered by IAccessible).
...
Each track item in Studio (including playlist viewer found in local and Remote Studio), and in extension, tracks found in Creator, Track Tool, and Remote VT client, consists of a row of columns (6 for Studio 5.0x and earlier, 18 for playlist viewer in Studio 5.10 and later, may vary on other track lists). As far as NVDA is concerned, it is an overlay class that provides a number of services, including:
...
As described in the previous section, Studio app module defines three classes for track items: an abstract base class for tracks found in Studio and other apps, a class representing track items found throughout Studio, and another for representing playlist viewer track items. Remote Studio app module adds a fourth class based on local Studio's playlist viewer item devoted to working with Remote Studio track items. In reality, all that matters is the first two, with the playlist viewer item class providing custom routines on top of the Studio track item class. In case of MRO, playlist viewer item class will be consulted if playlist viewer is in use, otherwise main Studio track item class will be consulted, which in turn will consult SPL track item base class. Other apps such as Track Tool and Remote VT client have their own MRO, ultimately consulting SPL track item base class defined in Studio app module.

The most important job of appModules.splcommon.splbase.SPLTrackItem class is announcing column data. Prior to 2020, it also housed column navigation commands and routines such as next and previous column, handler for Track Dial (historical), and a dedicated routine to announce column information given column number and optional header. In 2020, only Columns Explorer remains.
